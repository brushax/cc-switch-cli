...
管理配置的 SQL 备份/恢复与完整 JSON 快照导出。

**功能：** 自定义 SQL 备份命名、交互式备份选择、自动轮换（保留 10 个）、SQL 备份导入/导出、可读完整 JSON 快照导出、通用配置片段、WebDAV 同步。
...
# 可读 JSON 快照导出
cc-switch config export-json <path>  # 导出当前配置、settings.json 与 skills 索引快照（格式化 JSON）
